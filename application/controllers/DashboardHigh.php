...
class DashboardHigh extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($res == 'true' && $name) {
            ...
            $sql = "SELECT e.*,r.* FROM employee e,roles r WHERE e.role_id=r.role_id AND role_designation LIKE '%region%'";
            ...
            if (isset($_GET['month'])) {
                ...
                $sql2 = "SELECT o.*,e.emp_name FROM order_detail o,employee e WHERE o.emp_id = e.emp_id AND YEAR(Date) = '$year' AND MONTH(Date) = '$month' ORDER BY order_id DESC LIMIT 20 ";
            ...
            $this->load->view('high_pages/dashboard', $data);
        } else
    ...
    public function view_profile()
    {
        ...
        if ($res == 'true') {
            ...
            if (isset($_GET['id'])) {
                ...
                $id = $_GET['id'];
                $sql = "SELECT e.*,r.*,CONCAT(address_area,', ',address_city)  AS address FROM employee e,roles r,address a WHERE e.emp_id = '$id' AND e.role_id=r.role_id AND e.address_id=a.address_id AND r.role_designation LIKE '%employee%'";
                $query = $this->db->query($sql);
                if ($query->num_rows() > 0) {
                    ...
                    $data['admin_pic'] = $pic;
                    $data['admin_desig'] = $type;
                    $this->load->view('high_pages/view_profile', $data);
                } else
    ...
    public function view_map()
    {
        ...
        if ($res == 'true') {
            ...
            $this->load->view('high_pages/map', $data);
        } else
            redirect('http://localhost/ets1/index.php/login/index?invalid=true');
    }
    ...
    public function view_orders()
    {
        ...
        if ($res == 'true') {
            ...
            if (isset($_POST['month']) &&
                ...
                if ($month == 0) {
                    $sql = "SELECT o.*,e.emp_name FROM order_detail o,employee e WHERE o.emp_id = e.emp_id AND YEAR(Date) = '$year' ORDER BY order_id DESC";
                    $query = $this->db->query($sql);
                    ...
                    $sql = "SELECT o.*,e.emp_name FROM order_detail o,employee e WHERE o.emp_id = e.emp_id AND MONTH(Date) = '$month' ORDER BY order_id DESC";
                    $query = $this->db->query($sql);
                    ...
                    $sql = "SELECT o.*,e.emp_name FROM order_detail o,employee e WHERE o.emp_id = e.emp_id AND YEAR(Date) = '$year' AND MONTH(Date) = '$month' ORDER BY order_id DESC";
                    $query = $this->db->query($sql);
                    $data['month'] = $month;
                    $data['year'] = $year;
                    $data['orders'] = $query->result_array();
                ...
                $sql = "SELECT o.*,e.emp_name FROM order_detail o,employee e WHERE o.emp_id = e.emp_id ORDER BY order_id DESC LIMIT 50 ";
                $query = $this->db->query($sql);
                $data['month'] = $month;
                $data['year'] = $year;
                $data['orders'] = $query->result_array();
            ...
            $data['admin_pic'] = $pic;
            $data['admin_desig'] = $type;
            $this->load->view('high_pages/view_orders', $data);
        } else
            redirect('http://localhost/ets1/index.php/login/index?invalid=true');
    }
    ...
    public function view_employee_region()
    {
        $res = $this->session->userdata('login');
        $name = $this->session->userdata('name');
        $pic = $this->session->userdata('profile_pic');
        ...
        if ($res == 'true') {
            if (isset($_GET['id'])) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $query2 = $this->db->query($sql2);
                    $data['users_area'] = $query2->result_array();

                    $data['users_emp'] = array();
                    foreach ($query2->result_array() as $row) {
                        $tid = $row['emp_id'];
                        $sql3 = "SELECT * FROM employee WHERE sup_id='$tid'";
                        $query3 = $this->db->query($sql3);
                        $array1 = $data['users_emp'];
                        $array2 = $query3->result_array();
                        $data['users_emp'] = array_merge($array1, $array2);
                    }

                    $data['admin_name'] = $name;
                    $data['admin_pic'] = $pic;
                    $data['admin_desig'] = $type;
                    $this->load->view('high_pages/view_employee_region', $data);
                } else
                    redirect('http://localhost/ets1/index.php/dashboardhigh/index');
        ...
        if ($res == 'true') {
            if (isset($_GET['id'])) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $data['admin_pic'] = $pic;
                    $data['admin_desig'] = $type;
                    $this->load->view('high_pages/view_employee_area', $data);
                } else
                    redirect('http://localhost/ets1/index.php/dashboardhigh/index');
    ...
    public function edit_admin()
    {
        ...
        if ($res == 'true') {
            ...
            if (isset($_POST['id']) &&
                ...
                redirect('http://localhost/ets1/index.php/dashboardhigh/view_me');
        ...
        if ($res == 'true') {
            ...
            if ($admin) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $data['admin_pic'] = $pic;
                    $data['admin_desig'] = $type;
                    $this->load->view('high_pages/view_me', $data);
                } else
                    redirect('http://localhost/ets1/index.php/dashboardhigh/index');
            } else
                redirect('http://localhost/ets1/index.php/dashboardhigh/index');
        ...
        if ($res == 'true') {
            ...
            if ($adminid) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $data['admin_pic'] = $pic;
                    $data['admin_desig'] = $type;
                    $this->load->view('high_pages/edit_me', $data);
                } else
                    redirect('http://localhost/ets1/index.php/dashboardhigh/index');
    ...
    public function edit_employee()
    {
        ...
        if ($res == 'true') {
            ...
            if (isset($_POST['id']) &&
                ...
                $this->db->where('emp_id', intval($id));
                $this->db->update('employee', $info);
                redirect('http://localhost/ets1/index.php/dashboardhigh/view_profile?id=' . $id);
        ...
        if ($res == 'true') {
            ...
            if (isset($_GET['id'])) {
                ...
                if ($query->num_rows() > 0) {
                    ...
                    $data['admin_pic'] = $pic;
                    $data['admin_desig'] = $type;
                    $this->load->view('high_pages/edit_profile', $data);
                } else
                    redirect('http://localhost/ets1/index.php/dashboardhigh/index');
    ...
    public function add_user()
    {
        ...
        if ($res == 'true') {
            ...
            $this->load->view('high_pages/add_user', $data);
        } else
            redirect('http://localhost/ets1/index.php/login/index?invalid=true');
    }
